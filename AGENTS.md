...
| **米** | 币安#2 大资金(1448U) | audit/hybrid_live.py | 纯LONG臂(9/8晚关SHORT侧): 125U/5x/SL8/72h/TOP10L | 2026-09-08 |

命名规则(用户偏好): 果=小资金试验田(先试错), 米=大资金主粮仓(后放量)。代号与本文件§0状态快照联动维护。

## 0. 系统一句话

**地球版量化交易系统**:XGBoost 双模型(LONG/SHORT)每日全量重训,基于 946 维特征(主传感器=对BTC残差)预测币安 U-M 合约 2 日涨跌,aligned 时序(预测日=入场日)。**出场结构分两代, 勿混用**: 现行实盘(果/米 双账户, 9/3~9/8 起) = **LONG 无止盈 / SL-8% / 72h 滚动**; 老双模型通道 = TP+10%/SL-5%/48h, 但该通道已关闭(`TRADING_ENABLED=false`, 见§0), 其口径仅存于历史台账与影子臂对照。三端架构:生产端(本机,每日训练+预测+公证)/ GPU 回溯端(175.155.64.171,回测实验)/ 观察端(已下线)。

**当前状态(2026-09-06)**: 生产流水线完全体运行中(采集→训练→预测→公证→实盘→影子结算→晨报→体检, 全表见§4)。**交易分三层, 勿混淆**: ① 老双模型自动交易**关闭**(`backtester/config/current_params.json` 的 `_live_trading.TRADING_ENABLED=false` 覆盖代码默认值, 且本金<10u 跳过); ② **果实盘·主LONG臂运行中**(`audit/residual_live.py`, 9/2 起正式批; **2026-09-13 起选币由残差榜 top10_long_residual 切换为主LONG榜 top10_long** — 依据生产级8天对照[两模型真实选币 × 官方1m × SL-8%/72h, 300U名义]: 主LONG +3.04U/笔 vs 残差 -0.68U/笔, 主赢7/8天; 残差臂转为纯影子继续采集): 40U 名义/5x 逐仓/SL-8%(9/7由5%调)/72h/每日≤10笔, 交易所真实挂 SL algo 单; ③ **米实盘·纯LONG臂(第二账户)运行中**(`audit/hybrid_live.py`, 2026-09-08部署当日四次拍板定稿: 1448U(1万CNY), 125U名义/5x逐仓(提额至10单LONG止损≈101U=用户承受线; 杠杆2x→5x因SL8拉长持仓→并发峰值~26-30笔, 镜像果5x+SL8已验证包络51笔零强平, 85%守卫容量49笔)/SL-8%(对齐果账户)/72h/TOP10L/**SHORT侧9/8晚关闭**(直觉+数据双确认: S5影子36天与LONG日相关+0.07无对冲价值、后半段E归零、加SHORT使最差日-66→-88U回撤更深; 证伪线=S5滚动20天日均E>+2U或崩盘regime corr转负, 10/23终审复核; 影子S5照常跟踪)/cron 08:23 trade(pred落地即开, wait_pred 1800s)+每小时:41, 凭证.env HYBRID_BINANCE_*, state已入同步白名单)。
> ⚠️ **自 2026-09-13 起 果与米选币同源**(都用主LONG榜) → 两账户成为同一策略的大小两档, 集中度上升、且"防守型"敞口消失(残差臂只存在于影子)。
...
- **实盘**: 果实盘(`audit/residual_live.py`)9/2 起真实交易(币安 U 本位逐仓), **2026-09-13 起选币=主LONG榜**; 每日 08:21 对账→平到期→开新批(≤10笔), 72h 滚动; SL 交易所 algo 单 + 每小时 :31 对账重挂。米盘(第二账户)同结构、选币同为 top10_long, 仅名义不同(125U)
...
08:23  hybrid_live.py trade 米账户实盘: 同款流程(≤10笔, 125U名义/5x逐仓/SL-8%/72h, wait_pred 1800s)
...
每小时:31  residual_live.py reconcile 果账户对账/SL丢失重挂 | :41 hybrid_live.py reconcile 米账户同款
...
## 8.1 TOP10 全开回测模拟器规范(2026-09-07 立项, 对齐果实盘/米盘结构口径)

> 目的: FEAT4 组内条件特征的**公平审判** — TOP1+TP10 结构对为尾部设计的特征是"砍尾部的裁判"。模拟器对齐实盘执行器(果 audit/residual_live.py / 米 audit/hybrid_live.py)的**结构与执行**行为, 六条边界:
