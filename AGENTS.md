...
> 最后更新: 2026-09-12(实盘口径对齐 SL-8%/72h + 补米账户与影子2×2变体档 cron + 影子账面funding口径修正回刷) | 适用目录: `/home/myuser/websocket_new/`(earth-1.0 仓库)
...
| 代号 | 账户 | 执行器 | 结构 | 上线 |
|---|---|---|---|---|
| **果** | 币安#1 小资金(300U级) | audit/residual_live.py | 残差LONG臂: 40U/5x/SL8/72h/TOP10 | 2026-09-02 |
...
**当前状态(2026-09-06)**: 生产流水线完全体运行中(采集→训练→预测→公证→实盘→影子结算→晨报→体检, 全表见§4)。**交易分三层, 勿混淆**: ① 老双模型自动交易**关闭**(`backtester/config/current_params.json` 的 `_live_trading.TRADING_ENABLED=false` 覆盖代码默认值, 且本金<10u 跳过); ② **残差实盘 LONG 臂运行中**(`audit/residual_live.py`, 9/2 起正式批: 40U 名义/5x 逐仓/SL-8%(9/7由5%调,依据TOP10模拟器网格)/72h/每日≤10笔, 交易所真实挂 SL algo 单); ③ **米实盘·纯LONG臂(第二账户)运行中**(`audit/hybrid_live.py`, 2026-09-08部署当日四次拍板定稿: 1448U(1万CNY), 125U名义/5x逐仓(提额至10单LONG止损≈101U=用户承受线; 杠杆2x→5x因SL8拉长持仓→并发峰值~26-30笔, 镜像果5x+SL8已验证包络51笔零强平, 85%守卫容量49笔)/SL-8%(对齐果账户)/72h/TOP10L/**SHORT侧9/8晚关闭**(直觉+数据双确认: S5影子36天与LONG日相关+0.07无对冲价值、后半段E归零、加SHORT使最差日-66→-88U回撤更深; 证伪线=S5滚动20天日均E>+2U或崩盘regime corr转负, 10/23终审复核; 影子S5照常跟踪)/cron 08:23 trade(pred落地即开, wait_pred 1800s)+每小时:41, 凭证.env HYBRID_BINANCE_*, state已入同步白名单)。幽灵问题已修复(8/3, §7.1); 公证 stash-pop 冲突已根治(9/6, §7.10); 全链路体检 SKILL 上线(9/6, §4/§8)。
...
08:21  residual_live.py trade 果账户实盘: 对账/平到期 → 开新批(≤10笔, 40U名义/5x逐仓/SL-8%/72h)
...
## 8.1 TOP10 全开回测模拟器规范(2026-09-07 立项, 对齐实盘残差臂)

> 目的: FEAT4 组内条件特征的**公平审判** — TOP1+TP10 结构对为尾部设计的特征是"砍尾部的裁判"。模拟器完全对齐实盘 residual_live.py 行为, 六条边界:
