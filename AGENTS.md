...
> 最后更新: 2026-09-08(第二账户HYBRID上线+账户命名体系: 果/米) | 适用目录: `/home/myuser/websocket_new/`(earth-1.0 仓库)
...
12. **GPU回测引擎口径声明(2026-09-07 确立, TOP1方案已过时)**: `gpu_backtest_exp.py` 原生引擎为 **TOP1结构**(每日多空各取置信度最高1笔二选一, TP10/SL5, 180天=180笔) — 与实盘结构(TOP10 LONG全开/无TP/SL5%/72h滚动)不一致。**历史所有 GPU 实验水位(含干净基线16.81)都是 TOP1 口径**, 只可用于特征 A/B 相对排序, 不可当实盘预期。任何"结构级"结论(右尾统计/持有策略/仓位结构)必须用 TOP10 模拟器口径。
...
| 2 | SL-5%盘中触发(挂交易所逐仓) | 逐日扫low≤entry×0.95(含入场日) |
