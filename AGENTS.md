...
| **米** | 币安#2 大资金(1448U) | audit/hybrid_live.py | 3.8双臂: 125U/5x/SL8(LONG)+SL5(SHORT, 9/8拍板)/SHORT TP10/72h/TOP10L+TOP5S | 2026-09-08 |
...
**当前状态(2026-09-06)**: 生产流水线完全体运行中(采集→训练→预测→公证→实盘→影子结算→晨报→体检, 全表见§4)。**交易分三层, 勿混淆**: ① 老双模型自动交易**关闭**(`backtester/config/current_params.json` 的 `_live_trading.TRADING_ENABLED=false` 覆盖代码默认值, 且本金<10u 跳过); ② **残差实盘 LONG 臂运行中**(`audit/residual_live.py`, 9/2 起正式批: 40U 名义/5x 逐仓/SL-8%(9/7由5%调,依据TOP10模拟器网格)/72h/每日≤10笔, 交易所真实挂 SL algo 单); ③ **HYBRID 3.8双臂实盘(第二账户)运行中**(`audit/hybrid_live.py`, 2026-09-08部署: 1448U(1万CNY), 125U名义/5x逐仓(9/8晚两次拍板: 提额至10单LONG止损≈101U=用户承受线; 杠杆2x→5x因SL8拉长持仓+双臂→并发峰值~35笔, 2x容量19笔不够, 镜像果5x+SL8已验证包络51笔零强平, 85%守卫容量49笔)/SL-LONG 8%(9/8对齐果账户)+SHORT 5%(卖右尾结构保持)/SHORT TP10/72h/TOP10L+TOP5S, cron 08:27+每小时:41, 凭证.env HYBRID_BINANCE_*, state已入同步白名单)。幽灵问题已修复(8/3, §7.1); 公证 stash-pop 冲突已根治(9/6, §7.10); 全链路体检 SKILL 上线(9/6, §4/§8)。
