...
> ⚠️ **文档状态(2026-08-02)**: 本文件部分章节停留在地球版 1.0~1.3 时代, **权威现状以 Obsidian 知识库为准**(`/home/myuser/Sync/rainbow/` 系统/ + 研究/); 系统接手请先读 **`AGENTS.md`**。
> 最后更新: 2026-08-02 (**完全体**: 946维 / winsor 0.1% / 无闸门 / SOUP / 前向观察期 7/30~8/6)
...
| 参数 | 默认值 | 含义 |
|------|--------|------|
| `STOP_LOSS_PCT` | **5.0**(实盘 _live_trading) | 止损百分比(旧默认 10 为回测参数默认, 实盘 7/19 起 5%) |
| `TAKE_PROFIT_PCT` | 10.0 | 止盈百分比(TP20 为下轮候选) |
| `PROB_THRESHOLD` | 60.0 | 最低置信度阈值(动态自适应: 连续3天0%命中→100%禁止; 7天均值<15%→70%; ≥35%→55%) |
| `LEVERAGE` | **10**(7/19 从 20x 降) | 杠杆倍数 |
| `TRADING_ENABLED` | **false**(前向观察期 7/30~8/6) | 只训练预测不开仓 |
| `SOUP_ON` | **true**(7/30 上线) | 时间集成: 今日+近2日模型概率平均 |
| `LONG_MOM_FILTER` | **false**(7/30 摘除) | 动量闸门(已证伪为追涨接盘过滤器) |
...
> 完整参数卡见 Obsidian `系统/参数卡.md`; 回测水位(2026-08-02 修正): **Sharpe 17.29 / +1050% / MaxDD 15.6% / 76%** 为生产同源特征稳定水位, 非绝对收益承诺
...
- **⚠️ 生产训练特征偏差(2026-08-01 起, 调查中)**: `~/.local/share/auto_trade/train_data_latest.npz` 的特征与代码重建不一致(0GUSDT 列0: 理论 -1.9970 vs npz -0.2601); 审计 cron 已挂(8:04 快照/8:25 校验 → `logs/audit.log`), 8/3 出判定; 详见 AGENTS.md「已知问题」+ Obsidian 调查文档
- **回测结论修正(2026-08-02)**: 回测自训模型输出饱和(90~100%)是固有分布非 bug; 180d 真实水位 17.29 为生产同源特征稳定水位; 回测绝对水位不可作生产预期, 只用于 A/B 相对排序
...
| 2026-07-31 | **⚠️ 重大口径修正(GPT审计发现+独立复核确认): 回测豁免入场日当天止损** — 模拟从ki+1才检查止盈/止损, 入场日-5%波动豁免, 36/180笔(20%)当天已止损却记盈利, 虚增~17pp胜率; gpu_backtest_exp/gpu_frozen_test修复(off∈{0,1,2})并重跑: **无闸门42.45→20.46(79%) / 完全体44.57→17.47(76%) / 冻结93%→78%(19.32) / TP30臂37.64→21.53(71%)**; SOUP修正后不再加分(需重评估); alpha真实但水位降至Sharpe 17~20(仍~2倍基线); 前向观察期权重进一步上升 |
...
## 15. 2026-08-01~02 调查记录(追加)

> 详细: Obsidian `研究/前向观察与回测生产差异调查-20260802.md` | `回溯日志/001_946D_180d_GPU.md` | 本仓库 `AGENTS.md`

| 日期 | 事件 |
|---|---|
| 08-01 | 前向观察期 TOP1 4 连止损(-20%), 与回测水位(17~20)严重背离 → 启动全链路调查; 前向结算口径修正 v2(8:21 入场+1m+成交即盯盘); 发现 verify_yesterday 早盘快照偏置(邮件/tracker 收益乐观) |
| 08-02 | 数据全量同步(MD5 全一致)+ 回测引擎改 adt 生产同源构建; 180d 重跑 **Sharpe 17.29**(脏数据 18.68 vs 干净 17.29 几乎一致 → 回测 17~18 为稳定水位, 8/1"两套构建"结论作废); **发现生产 npz 训练特征与代码重建不一致**(0GUSDT -0.2601 vs -1.9970, 8/1 起); 挂审计 cron(8:04 快照/8:25 校验), 8/3 出判定 |
| 08-02 | 观察期 Top10 修正结算: LONG +2.6%(20笔) / SHORT -20%(10笔); 结论: 市场存在可统计 alpha(实证), 生产能否拿到待偏差修复+前向验证 |
