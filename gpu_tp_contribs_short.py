...
def trade_long(sym, ts):
    ...
    for off in (1, 2):
        k = kls[ki + off]
        if k['h'] >= ep * (1 + STOP_LOSS/100): return 'stop'
        if k['l'] <= ep * (1 - TAKE_PROFIT/100): return 'take'
    return 'hold'
...
log(f'WF {len(eval_days)} 天, 每日1个SHORT模型 + 逐笔贡献...')
...
for di, pred_ts in enumerate(eval_days):
    ...
    for ts in sdays[max(0, d_idx - TRAIN_WINDOW):d_idx - 2]:
        ...
        dd = np.load(fp)
        Xg.append(dd['feats']); yg.append(dd['labels'][:, 1])
    if not Xg: continue
