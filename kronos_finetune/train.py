...
for epoch in range(args.epochs):
    ...
    for ctx, tgt in pbar:
        ...
        s2_tgt = tgt_tokens[1]  # second-stream target
        s1_full = torch.cat([s1_ctx, s1_tgt], dim=1)        # (B, ctx_len + pred_len)
        s2_full = torch.cat([s2_ctx, s2_tgt], dim=1)

        # Forward with teacher forcing: predict s1 and s2 at each position
        ...
        # s2 loss — previously discarded (CRITICAL-KRONOS-002 fix)
        loss_s2 = criterion(
            s2_logits[:, -args.pred_days:].reshape(-1, s2_logits.size(-1)),
            s2_tgt.reshape(-1)
        )
