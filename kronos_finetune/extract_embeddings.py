...
# 优先 Kronos-small (与生产推理一致), 回退 base
_local_small = os.path.join(os.path.dirname(__file__), 'kronos_pretrained/Kronos-small')
_local_base = os.path.join(os.path.dirname(__file__), 'kronos_pretrained/Kronos-base')
LOCAL_MODEL = _local_small if os.path.exists(_local_small) else _local_base
LOCAL_TOKENIZER = os.path.join(os.path.dirname(__file__), 'kronos_pretrained/Kronos-Tokenizer-base')

# 加载模型
print(f"加载 Kronos: {LOCAL_MODEL}...")
...
CONTEXT_DAYS = 256
MIN_START = 256  # 必须 ≥ CONTEXT_DAYS, 否则空切片崩溃 (CRITICAL-GPU-002)
...
for i in pbar:
    ts = int(timestamps[i].timestamp())
    ctx_data = data_norm[i - CONTEXT_DAYS + 1:i + 1]
...
output = {
    ...
    'data_mean': mean.squeeze(0).tolist(),
    'data_std': std.squeeze(0).tolist(),
