...
MIN_VOLUME = 500000; TRADE_COST = 0.5
...
# ============ 回测核心 ============
def train_and_predict_batch(train_ts_list, pred_ts, entry_ts, klines):
    X_train, yL, yS = [], [], []
    ...
    ml = XGBClassifier(**XGB_PARAMS, scale_pos_weight=(len(yL)-pL)/pL, random_state=42)
    if TRAIN_MOM_LONG:
        ...
        ml.fit(X_train, yL)
    ms = XGBClassifier(**XGB_PARAMS, scale_pos_weight=(len(yS)-pS)/pS, random_state=43)
    ms.fit(X_train, yS)
    ...
    pl = ml.predict_proba(X_pred)[:, 1] if ml is not None else np.zeros(len(X_pred))
    ...
    if max(lp, sp) < PROB_THRESHOLD: return None
...
def run_walk_forward(sdays, klines):
    START = max(30, len(sdays) - DAYS - 1)
    tasks = []
    for d in range(START, len(sdays)-1, STRIDE):
        if MODE == 'lag':
    ...
    for i, (train_ts, pred_ts, entry_ts) in enumerate(tasks):
        result = train_and_predict_batch(train_ts, pred_ts, entry_ts, klines)
...
def print_summary(trades, elapsed):
    ...
    sep = '='*60
    print(f'\n{sep}')
    print(f'回测完成 ({DAYS}d stride={STRIDE} MODE={MODE})')
...
if __name__ == '__main__':
    ...
    if trades and summary:
        out = f'{HOME}/websocket_new/data/gpu_backtest_v5_{MODE}_{DAYS}d.json'
