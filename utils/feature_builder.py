...
def assemble_feature_vec(
    ...
    tr_ratio=1.0, tbr=0.5, vol_raw=0.0, fund_raw=0.0,
):
    ...
    vol_feats = [tr_ratio, tbr, vol_raw, fund_raw]  # volfeat(1.1) + fund_raw费率(1.2)
    return base + list(vol_col) + signals + list(rsi_div) + list(sector_feats) + list(macro_feats) + long_lookback + vol_feats
