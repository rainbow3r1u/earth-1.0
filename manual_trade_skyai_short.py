#!/usr/bin/env python3
"""一次性手动交易: SKYAIUSDT SHORT 10U 10x, 只挂 SL10 止损 (2026-08-12 08:05 执行)
用户 8/11 指令: 8/12 08:05 开 SKYAIUSDT SHORT, 10U 保证金, 10x, SL10 不挂止盈
...
def log(msg):
    ...
    with open(os.path.join(BASE, 'logs', 'manual_trade_skyai.log'), 'a') as f:
        f.write(line + '\n')
...
def main():
    ...
    log(f'SKYAIUSDT 价格={price}, 名义={notional}U, 数量={qty}')
    ...
    log(f'完成: SKYAIUSDT SHORT {filled}枚 @~{price}, SL={sl_price}, 无TP')
