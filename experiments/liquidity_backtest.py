...
def liq_score(kls_30d):
    ...
    r=[(h[i]-l[i])/c[i] if c[i]>0 else 0 for i in range(len(c))]
    rets=[(c[i]-c[i-1])/c[i-1] if c[i-1]>0 else 0 for i in range(1,len(c))]
    avg_r=np.mean(r)*100; vol_r=np.std(rets)*100 if rets else 0
    av=np.mean(v); cv=np.std(v)/av if av>0 else 999
    return (avg_r*0.4+vol_r*0.4)*math.log(max(av,1))/(1+cv)
...
def precompute(full):
    ...
    for di, day_ts in enumerate(all_days):
        ...
        if di%500==0: log(f'  排名: {di}/{len(all_days)} days')
    log(f'  排名完成: {len(all_days)}d, union={len(union)} symbols')
    return vrank,lrank,union,all_days

def build_samples(klines_f, oi_data, smap, sheats, btc_rets):
    all_s=[]
    for si,(sym,kls) in enumerate(klines_f.items()):
        if si%100==0: log(f'  样本: {si}/{len(klines_f)} coins')
        ...
        for i in range(25,n-2):
            ...
            try:
                ...
                vf=max(v20,0.002); r1n=round(r1/vf,4); r3n=round(r3/(vf*1.732),4); r5n=round(r5/(vf*2.236),4)
                ...
                mf=dp._get_macro_features(ts[i])
                mf=dp._apply_chain_tvl(mf,sym,ts[i])
                ...
                all_s.append((ts[i],sym,feat,ll,ls,nr*100))
            except: continue
    return all_s
...
def run_bt(by_day, recent_days, klines, rankings, label):
    ...
    for di, pred_ts in enumerate(recent_days):
        if di%50==0: log(f'  {label} 回测: {di}/{len(recent_days)} days')
        ...
        ml=XGBClassifier(n_estimators=200,max_depth=5,learning_rate=0.05,
            scale_pos_weight=(len(y_l)-pls)/pls,random_state=42,eval_metric='logloss',verbosity=0,n_jobs=6)
        ml.fit(X_tr,y_l)
        ms=XGBClassifier(n_estimators=200,max_depth=5,learning_rate=0.05,
            scale_pos_weight=(len(y_s)-pss)/pss,random_state=43,eval_metric='logloss',verbosity=0,n_jobs=6)
        ms.fit(X_tr,y_s)
        ps=by_day[pred_ts]
...
def summ(trades,label):
    ...
    log(f'{label}: {len(trades)}笔 Sharpe={sharpe:.2f} 累计={cum:+.1f}% 胜率={wr:.1f}% 止损={stops} 回撤={max_dd:.1f}%')
    return sharpe,cum,wr,stops,max_dd
...
def main():
    ...
    log("[2/5] 预计算排名...")
    vrank,lrank,union,all_days=precompute(full)
    
    log(f"[3/5] 构建样本 ({len(union)} coins)...")
    kf={s:full[s] for s in union if s in full}
    import requests
    try:
    ...
    except: fsyms=list(kf.keys())
    oi_data=dp.fetch_oi(fsyms) if hasattr(dp,'fetch_oi') else {}
    ...
    sheats=dp._precompute_sector_heats(kf,smap) if smap else {}
    ...
    btc_kls=kf.get('BTCUSDT',[]); btc_c=[k['c'] for k in btc_kls]
    btc_rets=dp._compute_returns(btc_c) if len(btc_c)>1 else []
    
    all_s=build_samples(kf,oi_data,smap,sheats,btc_rets)
    ...
    log(f"[4/5] Before: Top{TOP_N} 按量...")
    tb=run_bt(by_day,rd,kf,vrank,"Before(Vol)")
    s1,c1,wr1,st1,dd1=summ(tb,"Before(Vol)")
    
    # 中间保存，防止After崩溃丢失Before数据
    rd2=os.path.join(os.path.dirname(__file__),'..','data')
    os.makedirs(rd2,exist_ok=True)
    with open(os.path.join(rd2,'liquidity_backtest.json'),'w') as f:
        json.dump({'before':{'sharpe':round(s1,2),'cum':round(c1,2),'trades':len(tb),'wr':round(wr1,1),'stops':st1},'after':{},'status':'before_done'},f,indent=2)
    log("  Before结果已中间保存")
    
    log(f"[5/5] After:  Top{TOP_N} 按流动性...")
    import gc; gc.collect()  # 释放Before的XGBoost内存
    ta=run_bt(by_day,rd,kf,lrank,"After(Liq)")
    s2,c2,wr2,st2,dd2=summ(ta,"After(Liq)")
    
    log(f"\n{'='*60}")
    log(f"  对比总结 (耗时 {(time.time()-t0)/60:.0f}min)")
    log(f"{'='*60}")
    log(f"  {'指标':<12} {'Before(量)':>14} {'After(流动性)':>14} {'变化':>10}")
    log(f"  {'总收益':<12} {c1:>+13.1f}% {c2:>+13.1f}% {c2-c1:>+9.1f}%")
    log(f"  {'Sharpe':<12} {s1:>14.2f} {s2:>14.2f} {s2-s1:>+9.2f}")
    log(f"  {'交易次数':<12} {len(tb):>14} {len(ta):>14} {len(ta)-len(tb):>+10}")
    log(f"  {'胜率':<12} {wr1:>13.1f}% {wr2:>13.1f}% {wr2-wr1:>+9.1f}%")
    log(f"  {'止损次数':<12} {st1:>14} {st2:>14} {st2-st1:>+10}")
    log(f"  {'最大回撤':<12} {dd1:>13.1f}% {dd2:>13.1f}% {dd2-dd1:>+9.1f}%")
    
    if s2>s1+0.5: log("  ✅ 流动性筛选更优")
    elif abs(s2-s1)<0.5: log("  ➡️ 两者接近")
    else: log("  ❌ 纯量排序更优")
    ...
    with open(os.path.join(rd2,'liquidity_backtest.json'),'w') as f:
        json.dump({
            'before':{'sharpe':round(s1,2),'cum':round(c1,2),'trades':len(tb),'wr':round(wr1,1),'stops':st1,'max_dd':round(dd1,1)},
            'after':{'sharpe':round(s2,2),'cum':round(c2,2),'trades':len(ta),'wr':round(wr2,1),'stops':st2,'max_dd':round(dd2,1)}
        },f,indent=2)
    log("结果已保存")
