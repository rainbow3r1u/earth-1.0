...
def section_verify(imgs=None):
    """3. 刘账户实盘趋势曲线图 —— **2026-09-18 方案A: 由 TOP10影子口径 换成真钱账户**.
...
def section_2x2():
    ...
    try:
        ...
        files = [('base',   'SL-5%/48h <b>(原基线)</b>', 'hybrid_tracker.json'),
                 ...
                 ('sl5h72tp15', 'SL-5%/72h+TP15(限48h) <b>(9/16下午起实盘现行)</b>',
                  'hybrid_tracker_sl5h72tp15.json'),
                 # 2026-09-16 上午: 实盘梯度TP 15/30/无 → 下午第2天降到 20% → 1530 冻结为反事实, 1520 为现行
                 ('sl5h72lad1530', 'SL-5%/72h+梯度TP(15/30/无) <b>(已冻结为反事实)</b>',
                  'hybrid_tracker_sl5h72lad1530.json'),
                 ('sl5h72lad1520', 'SL-5%/72h+梯度TP(15/20/无) <b>(已冻结为反事实)</b>',
        ...
        parts = []
        for key, nm in (('sl5h72tp15', 'TP15(ROE+75%, 9/16下午起实盘现行)'), ('sl5h72lad1520', '梯度15/20/无'), ('sl5h72lad1530', '梯度15/30/无'), ('sl5h72tp10', 'TP10(ROE+50%)'), ('sl8h72tp15', 'TP15@SL-8%'), ('sl8h72tp30', 'TP30@SL-8%(9/15)')):
            st_ = stats.get(key)
