...
VARIANTS = {
    ...
    #    ⚠️ 2026-09-16 下午用户把第2天由 +30% 下调到 +20% → 本档降级为**反事实对照**(冻结, 不删)
    'sl5h72lad1530': {'sl_pct': 0.05, 'hold_days': 3, 'file': 'hybrid_tracker_sl5h72lad1530.json',
                      'tag': 'SL-5%/72h+梯度TP(15/30/无) [9/16上午实盘, 已冻结为反事实]',
                      'tp_ladder': [(0, 24, 0.15), (24, 48, 0.30), (48, 72, None)]},
    # 2026-09-16 下午用户拍板: 第2天 +30%→+20%(原话"现在把第二天改到Tp20%就行了, 第三天放开无Tp")
    #   → 本档为 9/16 下午起的**实盘镜像**(果+刘同结构)。
    #   用户目标口径已明确: "把本金尽快打回去, 彩票不容易等" = 早期落袋速度优先, 不是含肥日总收益。
    #   备注(用户已知悉, 不再重复测算): 该目标下 20% 与 30% 的差别主要在"第2天落袋 vs 第3天到期"的分配,
    #   总额差异在噪声内; 而第1天(≤24h)是净流血时段(40天累计 −1040U, 止损205笔 vs 止盈48笔),
    #   真正贡献正收益的是第3天(71笔到期 +1329U)。
    'sl5h72lad1520': {'sl_pct': 0.05, 'hold_days': 3, 'file': 'hybrid_tracker_sl5h72lad1520.json',
                      'tag': 'SL-5%/72h+梯度TP(0~24h +15% → 24~48h +20% → 48h后不挂, 对齐实盘 9/16起)',
                      'tp_ladder': [(0, 24, 0.15), (24, 48, 0.20), (48, 72, None)]},
}
